create table if not exists public.ai_usage_events (
  ...
  run_id uuid references public.exam_processing_runs(id) on delete set null,
...
create index if not exists idx_ai_usage_events_organization_created_at
  on public.ai_usage_events(organization_id, created_at desc);
create index if not exists idx_ai_usage_events_exam_id on public.ai_usage_events(exam_id);
create index if not exists idx_ai_usage_events_run_id on public.ai_usage_events(run_id);
create index if not exists idx_ai_usage_events_response_id on public.ai_usage_events(response_id);
...
begin
  select status, intake_mode, organization_id
  into current_status, current_mode, current_organization_id
  from public.exams
  where id = p_exam_id
  for update;
  ...
  loop
    ...
    where public.exam_questions.exam_id = p_exam_id
      and public.exam_questions.organization_id = current_organization_id;
  ...
  if p_publish then
    ...
    where q.exam_id = p_exam_id
      and q.organization_id = current_organization_id
      and (
...
declare
  ...
  current_organization_id uuid;
begin
  select exam_id, status, organization_id
  into current_exam_id, current_run_status, current_organization_id
