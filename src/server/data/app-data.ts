...
function scopedDemoData(organizationId: string): AppData {
  ...
  return {
    ...
    aiUsageEvents: data.aiUsageEvents.filter((event) => event.organization_id === organizationId),
...
export async function getAppData(options: GetAppDataOptions = {}): Promise<AppData> {
  ...
  const [
    ...
    supabase.from("ai_usage_events").select("*").eq("organization_id", organizationId),
  ...
  return {
    ...
    aiUsageEvents: (aiUsageEvents.data ?? []).sort(byCreatedDescThenId),
